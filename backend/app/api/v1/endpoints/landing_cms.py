...
- Platform-specific click IDs (fbclid)
...
class SubscriberCreate(BaseModel):
    ...
    utm_source: Optional[str] = Field(
        None, max_length=100, description="UTM source (facebook, instagram, whatsapp)"
    )
...
def detect_platform(
    ...
    if utm_src in ("facebook", "fb", "instagram", "ig", "wa", "whatsapp", "meta"):
        return "meta"
...
async def create_subscriber(
    ...
    async with get_async_session() as session:
        ...
        has_click_id = bool(subscriber_data.fbclid)
