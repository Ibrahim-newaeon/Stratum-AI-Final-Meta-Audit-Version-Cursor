...
async def get_playbook(
    ...
    if drivers.get("Event Match Rate", {}).get("value", 100) < 85:
        playbook_items.append(
            PlaybookItemResponse(
                id=str(uuid.uuid4()),
                title="Enrich Meta CAPI User Data",
                description="Send hashed email and phone with Meta CAPI events to improve match rates by 15-25%",
                priority="critical",
                owner=None,
                estimatedImpact=8.5,
                ...
                platform="Meta",
                status="pending",
                actionUrl="https://business.facebook.com/events_manager",
            )
        )
    ...
    # Always suggest WhatsApp conversion tracking if not at perfect score
    if emq_data["score"] < 95:
        playbook_items.append(
            PlaybookItemResponse(
                id=str(uuid.uuid4()),
                title="Track WhatsApp Conversions via Meta CAPI",
                description="Send WhatsApp-driven conversions through Meta CAPI for full-funnel attribution",
                priority="low",
                owner=None,
                estimatedImpact=2.0,
...
async def update_playbook_item(
    ...
    response_data = PlaybookItemResponse(
        id=item_id,
        title="Enrich Meta CAPI User Data",
        description="Send hashed email and phone with Meta CAPI events to improve match rates",
        priority="critical",
        owner=updates.owner,
        estimatedImpact=8.5,
        estimatedTime="2-4 hours",
