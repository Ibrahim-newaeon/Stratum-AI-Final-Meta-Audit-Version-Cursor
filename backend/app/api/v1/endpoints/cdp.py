...
async def get_event_statistics(
    ...
    # EMQ score distribution (EMQ is stored 0-100; bucket into 10-point ranges).
    # The divisor must be an inline literal: a bind parameter makes PostgreSQL see
    # different expressions in SELECT vs GROUP BY and reject the query.
    emq_bucket = func.floor(CDPEvent.emq_score / literal_column("10"))
    emq_distribution_result = await db.execute(
        select(
            emq_bucket.label("bucket"),
            func.count(CDPEvent.id).label("count"),
        )
        .where(
            CDPEvent.tenant_id == tenant_id,
