...
4. Attribution Accuracy (15%) - Platform vs web analytics attribution alignment
...
def calculate_attribution_accuracy(metrics: PlatformMetrics) -> EmqDriverResult:
    ...
    Measures alignment between platform attribution and web analytics.
    """
    weight = 0.15
    ...
    if metrics.ga4_conversions > 0:
        ...
        conv_variance = 1.0  # 100% variance if analytics has 0 but platform has data
    else:
    ...
    if value >= 80:
        status = DriverStatus.GOOD
        details = f"Platform-analytics variance: {avg_variance*100:.1f}%"
    elif value >= 60:
        status = DriverStatus.WARNING
