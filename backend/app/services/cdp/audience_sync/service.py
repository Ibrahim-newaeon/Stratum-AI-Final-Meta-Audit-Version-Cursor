...
Orchestrates audience sync to Meta (Facebook, Instagram, WhatsApp).
...
class AudienceSyncService:
    """
    Unified service for syncing CDP segments to Meta Custom Audiences.
    """
            ...
            platforms[cred.platform]["ad_accounts"].append(
                {
                    ...
                    # Credential model has no ad_account_name column; it lives in config
                    "ad_account_name": (cred.config or {}).get("ad_account_name")
                    or cred.ad_account_id,
                }
            )
    ...
    def _get_connector(
        ...
        kwargs = {}
        if platform == SyncPlatform.META.value:
