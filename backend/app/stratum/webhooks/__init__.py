...
# =============================================================================
"""
FastAPI-based webhook server that receives real-time updates from Meta channels.
...
app = FastAPI(
    ...
    description="Receives real-time updates from Meta advertising channels",
    version="1.0.0",
)
...
async def ecommerce_webhook_receive(
    ...
    2. Server-side tagging container
...
async def process_ecommerce_event(event_data: dict[str, Any]):
    """
    Process e-commerce event and forward to Meta.

    This converts the incoming event to our unified format and sends
    to Meta (Facebook, Instagram, WhatsApp) via server-side tracking.
    """
