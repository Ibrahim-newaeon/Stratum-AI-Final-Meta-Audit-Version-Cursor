...
class TestConversionLatencyTracking:
    ...
    def test_record_latency_directly(self):
        ...
        stats = tracker.get_stats(platform="meta", event_type="send_to_ack")
    ...
    def test_percentiles(self):
        ...
        for i in range(100):
            tracker.record_latency("whatsapp", "pixel_to_capi", float(i + 1))

        stats = tracker.get_stats(platform="whatsapp", event_type="pixel_to_capi")
    ...
    def test_stats_by_platform(self):
        ...
        tracker.record_latency("meta", "test", 200)
        tracker.record_latency("whatsapp", "test", 250)

        by_platform = tracker.get_stats_by_platform()

        assert "meta" in by_platform
        assert "whatsapp" in by_platform
        assert by_platform["meta"].count == 3
    ...
    def test_pending_count(self):
        ...
        tracker.start_tracking("event_3", "whatsapp", "test")

        pending = tracker.get_pending_count()

        assert pending["meta"] == 2
        assert pending["whatsapp"] == 1
