...
- "Log in with Facebook" is authentication, not activation. It reuses `META_APP_ID`/`META_APP_SECRET` but asks only for `public_profile`/`email`, reads `GET /debug_token`, `GET /me` and - on Facebook Login for Business - `GET /oauth/access_token` to redeem the browser's single-use code, and nothing else, creates no `tenant_platform_connection`, and belongs in no platform enum. It is off by default (`FACEBOOK_LOGIN_ENABLED`). `FACEBOOK_LOGIN_CONFIG_ID`, when set, must name a Meta **User access token** configuration: a System User one (WhatsApp Embedded Signup, Conversions API onboarding, any "share business assets" template) opens a business-onboarding dialog rather than a consent dialog, and its token is refused as `not_a_user_token` because it identifies a business portfolio and not a person. The browser's `authResponse` is never trusted: `backend/app/services/meta/login_client.py` re-derives the identity server-side and refuses any token whose `app_id` is not ours - the code exchange changes only how the token is obtained, never how it is trusted. An email match alone must not adopt an existing password account - see docs/integrations/README.md.
...
- The autopilot write path is real but **disabled by default**. `backend/app/services/meta/write_client.py` is the only module that may issue a non-`GET` request to Meta; `backend/app/services/meta/action_executor.py` drives it, measuring the before- and after-values by reading the entity rather than assuming them. It requires the **`ads_management`** permission (subject to Meta App Review), and it writes nothing unless `autopilot_execution_enabled` is true **and** `autopilot_execution_dry_run` is false - both default to no write. `tasks.apply_actions_queue` also remains unscheduled: it has no beat entry and is absent from `celery_app.include`. Do not schedule it, widen `autopilot_executable_action_types` past `SAFE_ACTIONS`, or relax a guard rail without tracing every consumer, test and document together - see docs/architecture/trust-engine.md.
...
- Meta budget and bid fields are in the ad account's Meta API units, which are **not** this schema's `*_cents` units; they differ by 100x for a zero-decimal currency such as JPY. Convert only through `major_to_meta_minor` in `write_client.py`, which uses Meta's published currency offsets rather than ISO 4217 (the two disagree for HUF, IDR, TWD, BHD, JOD and others) and refuses an unknown currency instead of guessing. Which currency applies is decided by the ad account the **entity itself reports**, not by the tenant's first connected account, and the absolute budget floor/ceiling are configured per currency because one scalar cannot describe both a dollar and a yen budget.
